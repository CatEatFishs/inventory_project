...
//已出库存
class OutInventoryList extends StatefulWidget {
  final String goodName;

  const OutInventoryList({Key key, this.goodName = '冰箱'}) : super(key: key);

  @override
  _OutInventoryListState createState() => _OutInventoryListState();
}

class _OutInventoryListState extends State<OutInventoryList>
    with AutomaticKeepAliveClientMixin {
  @override
  bool get wantKeepAlive => true;

  List<OutGoodModel> goodList = [];
  StreamSubscription<InAndOutEvent> inAndOutEventStreamSubscription;

  int totalOutInventory = 0; //出库总数
  double totalPrice = 0.0; //总利润
              ...
              .map((index, model) => MapEntry(
                  ...
                  TableRow(decoration: BoxDecoration(), children: [
                    ...
                    Container(
                      ...
                      padding: EdgeInsets.only(left: 5),
                      alignment: Alignment.centerLeft,
                      child: Text('${goodList[index].outPrice}'),
                    ),
                    Container(
                      height: setWidth(90),
                      padding: EdgeInsets.only(left: 5),
                      alignment: Alignment.centerLeft,
                      child: Text(
                          '${goodList[index].outNum * (goodList[index].outPrice - goodList[index].price)}'),
                    )
                  ])))
              .values
              .toList()),
  ...
  void getCheck() async {
    ...
    queryOutInventoryData(provider);
  }

  //详细查询剩余库存
  queryOutInventoryData(DefaultProvider provider) async {
    goodList.clear();
    totalOutInventory = 0;
    totalPrice = 0.0;
    goodList = await provider.queryOutInventoryData(
        startTime: DateUtils.DatePaserToMils(goodStartTime),
        endTime: DateUtils.DatePaserToMils(goodEndTime));
    goodList.forEach((element) {
      totalOutInventory += element.outNum;
      totalPrice += ((element.outPrice - element.price) * element.outNum);
    });
